          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
              {([['pending','承認待ち'], ['approved','承認済み'], ['rejected','差し戻し済み'], ['all','すべて']] as const).map(([val, label]) => (
                <button key={val} onClick={() => { setFilterPS(val); setCheckedIds(new Set()); setExpandedId(null) }}
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid #F3F4F6', background: '#F9FAFB' }}>
                      ...
                      {['ステータス', '研究室 / 教員', '更新項目', '提案内容', '提案者', '日時', '操作'].map((h, i) => (
                        ...
                        <>
                          <tr key={p.id}
                            ...
                            <td style={{ padding: '10px 12px', maxWidth: 260 }}>
                              <p style={{ margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 12, color: '#374151' }}>
                                {p.new_value.length > 60 ? p.new_value.slice(0, 60) + '…' : p.new_value}
                              </p>
                            </td>
                            {/* 提案者 */}
                              ...
                              {p.status === 'pending' ? (
                                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                  <button
                                    ...
                                    disabled={isUpdating}
                                    style={{ padding: '5px 12px', borderRadius: 7, border: 'none', background: '#16A34A', color: 'white', fontSize: 11, fontWeight: 700, cursor: isUpdating ? 'wait' : 'pointer', fontFamily: font, whiteSpace: 'nowrap' }}>
                                    {isUpdating ? '…' : '✅ 承認'}
                                  </button>
                                  <button
                                    onClick={() => { setExpandedId(isExpanded ? null : p.id); setReviewNote('') }}
                                    disabled={isUpdating}
                                    style={{ padding: '5px 12px', borderRadius: 7, border: '1px solid #FCA5A5', background: '#FEF2F2', color: '#EF4444', fontSize: 11, fontWeight: 700, cursor: isUpdating ? 'wait' : 'pointer', fontFamily: font, whiteSpace: 'nowrap' }}>
                                    ↩️ 差し戻し
                                  </button>
                                </div>
                              ) : (
                                <button
                                  ...
                                  style={{ padding: '5px 10px', borderRadius: 7, border: '1px solid #E5E7EB', background: 'white', color: '#6B7280', fontSize: 11, cursor: 'pointer', fontFamily: font }}>
                                  {isExpanded ? '閉じる' : '詳細'}
                                </button>
                              )}
                            </td>
                          ...
                          {isExpanded && (
                            <tr style={{ borderBottom: '1px solid #F3F4F6' }}>
                              <td colSpan={8} style={{ padding: '0 12px 14px 48px', background: '#F8FAFF' }}>
                                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                                  ...
                                  {/* 差し戻し（pendingのみ） or レビューノート */}
                                  {p.status === 'pending' ? (
                                    <div style={{ flex: 1, minWidth: 200 }}>
                                      <p style={{ fontSize: 10, fontWeight: 700, color: '#9CA3AF', margin: '0 0 4px' }}>差し戻し理由（任意）</p>
                                      <textarea value={reviewNote} onChange={e => setReviewNote(e.target.value)} rows={3}
                                        placeholder="差し戻す場合のみ理由を入力..."
                                        style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid #E5E7EB', fontSize: 12, boxSizing: 'border-box', outline: 'none', fontFamily: font, resize: 'vertical', marginBottom: 8 }} />
                                      <button onClick={() => handleReject(p)} disabled={isUpdating}
                                        style={{ padding: '7px 16px', borderRadius: 8, border: '1px solid #FCA5A5', background: '#FEF2F2', color: '#EF4444', fontSize: 12, fontWeight: 700, cursor: isUpdating ? 'wait' : 'pointer', fontFamily: font }}>
                                        {isUpdating ? '処理中...' : '↩️ 差し戻しを確定する'}
                                      </button>
                                    </div>
                                  ) : p.review_note ? (
                                    <div style={{ flex: 1, minWidth: 200 }}>
                                      <p style={{ fontSize: 10, fontWeight: 700, color: '#9CA3AF', margin: '0 0 4px' }}>差し戻し理由</p>
                                      <p style={{ fontSize: 12, color: '#374151', margin: 0, background: '#FEF2F2', padding: '8px 10px', borderRadius: 8 }}>{p.review_note}</p>
                                    </div>
                                  ) : null}
                                </div>
                              </td>
