...
const FILTER_W = 220  // フィルターパネル幅
const FILTER_BTN_W = 44 // 漏斗ボタン幅+余白

const W = 3200, H = 2400
const MIN_ZOOM = 0.05, MAX_ZOOM = 3.0, ZOOM_STEP = 0.25

function clampOffset(ox: number, oy: number, zoom: number, svgW = 800, svgH = 600) {
  const contentW = W * zoom, contentH = H * zoom, PAD = 80
  const minX = Math.min(0, svgW - contentW - PAD), maxX = Math.max(0, PAD)
  const minY = Math.min(0, svgH - contentH - PAD), maxY = Math.max(0, PAD)
...
function calcFocusTransform(x: number, y: number, targetZoom = 2, svgW = 800, svgH = 600) {
  ...
  const ox = svgW / 2 - x * clamped, oy = svgH / 2 - y * clamped
...
export default function ExplorePage() {
  ...
  useEffect(() => {
    ...
    let tagCount = 0
    for (const [tag, ids] of tagSet) { if (tagCount >= 4) break; suggs.push({ type: 'tag', label: tag, sub: `${ids.length}件の研究室`, matchIds: ids }); tagCount++ }
  ...
  const placed = labs.map((lab, i) => ({ ...lab, x: lab.map_x ?? 100 + (i % 5) * 130, y: lab.map_y ?? 100 + Math.floor(i / 5) * 120 }))
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); commitSuggestion(suggIndex >= 0 ? suggestions[suggIndex] : suggestions[0]) }
  ...
  const toggleAllCourses = (dept: string) => {
    ...
    setSelectedCourses(prev => { const n = new Set(prev); allSelected ? keys.forEach(k => n.delete(k)) : keys.forEach(k => n.add(k)); return n })
  }
  ...
  const getLabCourseInfo = (labId: string) => {
    ...
    // ユニークな学科リスト
    const depts = [...new Set(entries.map(c => c.undergraduate_dept))]
    const courses = [...new Set(entries.map(c => c.course))]
    return { depts, courses }
  }
  ...
  const clusterEllipses = CLUSTER_NAMES.map((_, ci) => {
    ...
    const rx = (Math.max(...xs) - Math.min(...xs)) / 2 + 42, ry = (Math.max(...ys) - Math.min(...ys)) / 2 + 42
    return { cx, cy, rx, ry }
  })
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <span style={{
              ...
              color: chipColor, background: chipBg, padding: '2px 9px 2px 6px', borderRadius: 999, marginBottom: 4,
            }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: color, flexShrink: 0 }} />
              {clusterName}
            </span>
            {/* 研究室名 */}
            <p style={{ fontSize: 15, fontWeight: 700, margin: '0 0 1px', color: 'var(--text)', lineHeight: 1.3 }}>{lab.name}</p>
            <p style={{ fontSize: 12, color: 'var(--muted)', margin: '0 0 6px' }}>{lab.faculty_name}</p>

            {/* 学科・コース・専攻 */}
            {(courseInfo || lab.dept) && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 6 }}>
                {courseInfo?.depts.map((d, i) => (
                  <span key={i} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 999, background: 'rgba(99,102,241,0.10)', color: '#4F46E5', fontWeight: 500 }}>
                    🎓 {d}
                  </span>
                ))}
                {courseInfo?.courses.slice(0, 3).map((c, i) => (
                  <span key={i} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 999, background: 'rgba(99,102,241,0.06)', color: '#6366F1', fontWeight: 500 }}>
                    {c}
                  </span>
                ))}
                {lab.dept && (
                  <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 999, background: 'rgba(107,114,128,0.10)', color: '#6B7280', fontWeight: 500 }}>
                    🏫 {lab.dept}
                  </span>
                )}
              </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <button onClick={(e) => togglePin(lab.id, e)} title={isPinned ? 'ピンを外す' : 'ピン留め'} style={{
              ...
              background: isPinned ? '#FEF9C3' : '#F3F4F6', cursor: 'pointer', fontSize: 17,
  ...
  return (
    <>
      <style>{`
        ...
        .course-row:hover{background:rgba(59,130,246,.06)!important}
      ...
      <main style={{ position: 'fixed', inset: 0, overflow: 'hidden' }}>
                  ...
                  {(['lab', 'faculty', 'tag'] as const).map(type => {
                    ...
                    return (
                      <div key={type}>
                        ...
                        {group.map((s, i) => {
                          const globalIdx = suggestions.indexOf(s), isSelected = suggIndex === globalIdx
                          return (
                            <div key={i} className="sugg-item" onMouseDown={() => commitSuggestion(s)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', cursor: 'pointer', background: isSelected ? '#EFF6FF' : 'white', borderLeft: isSelected ? '2px solid #3B82F6' : '2px solid transparent' }}>
                              {suggIcon(s.type)}
                              <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div ref={mapDivRef} style={{ position: 'absolute', inset: 0, background: 'var(--card)', overflow: 'hidden', userSelect: 'none' }}>
                ...
                {placed.map(lab => {
                  ...
                  return (
                    <g key={lab.id} data-node="true" style={{ cursor: 'pointer' }}
                      ...
                      <circle cx={lab.x} cy={lab.y} r={r + (isActive || isFocused ? 3 : 2)} fill="white" filter={isFocused ? 'url(#ns-focus)' : isActive ? 'url(#ns-h)' : 'url(#ns)'} opacity={isMatch ? 1 : 0.2} />
                      ...
                      <text x={lab.x} y={lab.y + r + 15} textAnchor="middle" fontSize={10.5 / zoom} fontWeight={isActive || isFocused || isPinned ? 700 : 500} fill={isActive || isFocused ? color : isPinned ? '#92400E' : '#4B5563'} opacity={isMatch ? 1 : 0.18} style={{ pointerEvents: 'none' }}>{lab.name.slice(0, 14)}</text>
                    </g>
                  )
                })}
              </g>
          ...
          <div style={{ position: 'absolute', top: 120, left: 16, bottom: 72, zIndex: 20, display: 'flex', alignItems: 'flex-start', gap: 8, pointerEvents: 'none' }}>
              ...
              <div style={{
                ...
                <div style={{ padding: '10px 12px 8px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, color: hasFilter ? '#3B82F6' : 'var(--text)' }}>学科・コースで絞る</span>
                  </div>
                ...
                <div style={{ overflowY: 'auto', flex: 1 }}>
                  {deptList.map(dept => {
                    ...
                    const allSelected = selectedInDept === courses.length && courses.length > 0
                    return (
                      <div key={dept}>
                        <div className="dept-row" onClick={() => toggleDeptExpand(dept)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', cursor: 'pointer', background: 'transparent', borderBottom: '1px solid rgba(229,231,235,0.5)' }}>
                          ...
                          {selectedInDept > 0 && <span style={{ fontSize: 10, background: '#3B82F6', color: 'white', borderRadius: 999, padding: '1px 6px', flexShrink: 0 }}>{selectedInDept}</span>}
                        </div>
                        {isExpanded && (
                          <div style={{ background: 'rgba(249,250,251,0.8)' }}>
                            <label className="course-row" onClick={e => { e.preventDefault(); toggleAllCourses(dept) }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px 6px 28px', cursor: 'pointer', borderBottom: '1px solid rgba(229,231,235,0.3)' }}>
                              <input type="checkbox" checked={allSelected} onChange={() => toggleAllCourses(dept)} style={{ accentColor: '#3B82F6', width: 13, height: 13, flexShrink: 0 }} />
            ...
            {/* 漏斗FABボタン */}
            <button className="filter-fab" onClick={() => setFilterOpen(v => !v)} title={filterOpen ? 'フィルターを閉じる' : 'フィルターを開く'}
              style={{
                width: 36, height: 36, borderRadius: 10, border: 'none', flexShrink: 0,
                background: hasFilter ? 'rgba(59,130,246,0.15)' : 'rgba(255,255,255,0.94)',
                backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)',
                boxShadow: '0 2px 10px rgba(17,24,39,0.13)',
              ...
              <FilterIcon active={hasFilter} />
              {hasFilter && (
                <span style={{ position: 'absolute', top: -5, right: -5, background: '#3B82F6', color: 'white', borderRadius: 999, fontSize: 9, fontWeight: 700, padding: '1px 4px', lineHeight: 1.4 }}>
          ...
          {/* ── プレビューカード（フィルター幅を除いた範囲に表示） ── */}
          {preview && (
            <div style={{
              position: 'absolute', bottom: 16,
              left: filterLeftOffset,  // フィルターパネル分オフセット
              right: 16,
...
function highlightMatch(text: string, query: string) {
  ...
  return (
    <>{text.slice(0, idx)}<mark style={{ background: '#DBEAFE', color: '#1D4ED8', borderRadius: 2, padding: '0 1px' }}>{text.slice(idx, idx + query.length)}</mark>{text.slice(idx + query.length)}</>
  )
}