...
function ContactForm() {
  ...
  const initialLabName =
    searchParams.get('lab_name') ||
    searchParams.get('target_lab') ||
  ...
  const [prefillLocked, setPrefillLocked] = useState(!!initialLabId || !!initialLabName)
  ...
  useEffect(() => {
    ...
    setSelectedLabName(initialLabName)
    setPrefillLocked(!!initialLabId || !!initialLabName)
    setSelectedDept('')
  }, [initialType, initialLabId, initialLabName])

  useEffect(() => {
    if (!labs.length) return

    if (initialLabId) {
      const foundById = labs.find(l => String(l.id) === String(initialLabId))
      if (foundById) {
        setSelectedLabId(foundById.id)
        setSelectedLabName(foundById.name)
        if (foundById.dept) setSelectedDept(foundById.dept)
    ...
    if (initialLabName) {
      const normalizedInitialLabName = initialLabName.trim()
      const foundByName = labs.find(l => l.name.trim() === normalizedInitialLabName)
      if (foundByName) {
        setSelectedLabId(foundByName.id)
        setSelectedLabName(foundByName.name)
        if (foundByName.dept) setSelectedDept(foundByName.dept)
        setPrefillLocked(true)
        return
      }

      setSelectedLabId('')
      setSelectedLabName(normalizedInitialLabName)
      setPrefillLocked(true)
    }
  }, [labs, initialLabId, initialLabName])
  ...
  const handleLabSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedLabId(id)
    const found = labs.find(l => String(l.id) === String(id))
    setSelectedLabName(found?.name ?? '')
  }

  const showLockedLabField = inquiryType === 'correction' && prefillLocked && !!selectedLabName
  ...
  const handleSubmit = async () => {
    ...
    if (inquiryType === 'correction' && !selectedLabId && !selectedLabName) {
      setError('研究室を選択してください')
      return
    }
    if (inquiryType === 'correction' && !correctionField) {
      setError('修正対象の項目を選択してください')
      return
    }
    if (!body.trim()) {
